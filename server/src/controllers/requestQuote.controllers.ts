...
import { REQUEST_QUOTE_MESSAGE } from '~/constants/message.constants'
...
export const getRequestQuoteController = async (req: Request, res: Response) => {
  try {
    res.json({
      ...
      message: REQUEST_QUOTE_MESSAGE.GET_REQUESTQUOTE_SUCCESS,
      data: await requestQuoteService.get()
    })
  } catch {
    res.json({
      code: RESPONSE_CODE.GET_REQUESTQUOTE_FAILED,
      message: REQUEST_QUOTE_MESSAGE.GET_REQUESTQUOTE_FAILURE
    })
  }
}
...
export const requestQuoteController = async (
  ...
  try {
    ...
    res.json({
      ...
      message: REQUEST_QUOTE_MESSAGE.REQUESTQUOTE_SUCCESS
    })
  } catch {
    res.json({
      code: RESPONSE_CODE.REQUESTQUOTE_FAILED,
      message: REQUEST_QUOTE_MESSAGE.REQUESTQUOTE_FAILURE
    })
  }
}
...
export const completeRequestQuoteController = async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      message: REQUEST_QUOTE_MESSAGE.COMPLETE_REQUESTQUOTE_SUCCESS
    })
  } catch {
    res.json({
      code: RESPONSE_CODE.COMPLETE_REQUESTQUOTE_FAILED,
      message: REQUEST_QUOTE_MESSAGE.COMPLETE_REQUESTQUOTE_FAILURE
    })
  }
}
